...
def is_readelf_available():
    try:
        subprocess.run(["readelf", "--version"], check=True,
        ...
        with open(mcu_pkg_size, 'r') as file:
            for line in file:
                if "NUM_BANKS =" in line:
                    ...
                    match = re.search(
                        r"RAM(\d+)_SIZE = 32'h([0-9A-Fa-f]+);", line)
                    if match:
            ...
            for line in file:
                ...
                if collect:
                    ...
                    if len(parts) >= 4:
                        ...
                        attributes = parts[3]
                        sections[name] = {
                            'origin': origin, 'length': length, 'attributes': attributes}
                        index += 1
    except FileNotFoundError:
        print("File not found. Please check the path and try again.")
    return sections
...
def get_readelf_output(elf_file):
    ...
    try:
        result = subprocess.run(
            ['readelf', '-l', elf_file], capture_output=True, text=True)
...
def get_regions(program_headers, section_to_segment):
    ...
    data_sections = {'.power_manager', '.rodata', '.data',
                     '.sdata', '.sbss', '.bss', '.heap', '.stack'}
    ...
    for idx, ph in enumerate(program_headers):
        ...
        if any(sec in sections for sec in code_sections):
            ...
            # Special data handling like interleaved can be marked differently if needed
            region_type = 'i'
            name = 'IL data'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
            description="Parse ELF, MAP, and MCU package files for analysis."
        )
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to the ELF file (e.g., build/output.elf)"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to the MAP file (e.g., build/output.map)"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to the core_v_mini_mcu_pkg.sv file"
    )

    args = parser.parse_args()

# READ THE READELF OUTPUT AND PARSE TO OBTAIN THE DIFFERENT REGIONS
    output = get_readelf_output(args.elf_file)
...
# OBTAIN THE NUMBER AND SIZE OF THE BANKS
    num_banks, num_il_banks, bank_sizes_B = get_banks_and_sizes(args.mcu_pkg_file)
    total_size_B = sum(bank_sizes_B)
    print(f"Total space: {total_size_B/1024:0.1f} kB = Continuous:", [int(s/1024) for s in bank_sizes_B[:num_banks-num_il_banks]], "kB + Interleaved:", [
          int(s/1024) for s in bank_sizes_B[-num_il_banks:]] if num_il_banks else [0], "kB")
...
# If there are no IL banks, create an entry with length 0
    sections = get_memory_sections(args.map_file)
    try:
        sections['code'] = sections.pop('ram0')
        sections['data'] = sections.pop('ram1')
        sections['ildt'] = sections.pop('ram2') if num_il_banks else {
            'origin': sections['data']['origin'] + sections['data']['length'], 'length': 0}
    except:
        print("Memory distribution analysis not available for LINKER=flash_exec")
        quit()

# Compute the total space used for code and data
    total_space_used_code = sum(region['size_B']
                                for region in regions if region['name'] == 'code')
    total_space_used_data = sum(region['size_B']
                                for region in regions if region['name'] == 'data')
    total_space_used_ildt = sum(region['size_B']
                                for region in regions if region['name'] == 'IL data')
...
# Compute the total space required to store code and data
    ...
    min_code_start = min(region['start_add']
                         for region in code_regions) if code_regions else 0
    max_code_end = max(region['end_add']
                       for region in code_regions) if code_regions else 0
    total_space_required_code = max_code_end - min_code_start

    min_data_start = min(region['start_add']
                         for region in data_regions) if data_regions else 0
    max_data_end = max(region['end_add']
                       for region in data_regions) if data_regions else 0
    total_space_required_data = max_data_end - min_data_start

    min_ildt_start = min(region['start_add']
                         for region in ildt_regions) if ildt_regions else 0
    max_ildt_end = max(region['end_add']
                       for region in ildt_regions) if ildt_regions else 0
    ...
    for bank_idx, bank in enumerate(banks):
        ...
        if bank['type'] == "IntL":
            for piece in range(len(bank['use'])):
                ...
                bank['use'][piece] = '-'
                for region in regions:
                    used_by_others = (region['size_B'] *
                                      (num_il_banks-1)/num_il_banks)
                    if address >= region['start_add'] and address < region['end_add'] - used_by_others:
                        bank['use'][piece] = region['symbol']
                        utilization += granularity_B

        bank['use'] = ''.join([''.join(sublist) for sublist in bank['use']])
        print(bank['type'], bank_idx, bank['use'],
              f"\t{100*(utilization/bank['size']):0.1f}%")
